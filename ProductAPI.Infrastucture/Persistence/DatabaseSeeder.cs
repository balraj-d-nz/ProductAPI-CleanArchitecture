...
{
    /// <summary>
    /// Seeds Database with default Products and SystemUser.
    /// </summary>
    public static class DatabaseSeeder
    {
        public static async Task SeedDatabaseAsync(DatabaseContext context)
        {
            ...
            var systemUserId = "ProductApi-System";
            //Check to see if the SystemUser already exists
            var systemUser = await context.Users.FindAsync(systemUserId);
            if (systemUser == null)
            {
                //Create new SystemUser if it does not exist and Save it to DB.
                var defaultUser = new User()
                {
                    Id = systemUserId,
                    Email = $"{systemUserId}@email.com",
                    Name = systemUserId,
                    IsActive =  true
                };
            ...
            // Check if there is already Product data
